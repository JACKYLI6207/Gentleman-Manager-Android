...
$OutputApk = Join-Path $ProjectRoot "Gentleman-Manager-Android-v$version$apkSuffix.apk"

# 快速：只編 arm64 Rust + 單 ABI，Gradle 用 daemon；完整：四 ABI + --no-daemon
$script:GradleExtraProps = @()
if ($IsFastBuild) {
    $script:GradleExtraProps = @('-PabiList=arm64-v8a', '-PtargetList=aarch64', '-ParchList=arm64')
...
$buildLabel = if ($IsFastBuild) { '快速測試用 APK（arm64-v8a）' } else { '完整 APK（四架構）' }

if (-not $Quiet) {
    Write-Host ''
    Write-Host "=== Gentleman Manager Android — $buildLabel ===" -ForegroundColor Cyan
    Write-Host "輸出：$OutputApk"
    if ($IsFastBuild) {
        Write-Host '說明：僅適合多數實機驗證；模擬器 x86 或正式分發請用完整版。' -ForegroundColor DarkYellow
    }
    Write-Host ''
}
...
$signSec = Invoke-CommandWithProgress -PhaseName 'Sign' {
    ...
    Get-ChildItem (Join-Path $ProjectRoot 'Gentleman-Manager-Android-v*.apk.idsig') -ErrorAction SilentlyContinue |
        Remove-Item -Force -ErrorAction SilentlyContinue
    if ($IsFastBuild) {
        $fullApk = Join-Path $ProjectRoot "Gentleman-Manager-Android-v$version.apk"
